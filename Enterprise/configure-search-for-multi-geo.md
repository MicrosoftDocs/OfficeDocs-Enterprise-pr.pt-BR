...
Assim que o ambiente de Multi-Geo tiver sido configurado, os usuários verão notícias, sites recentes e visitados vários locais geo na sua home page do SharePoint. Se eles usarem a caixa Pesquisar na home page do SharePoint, eles somente obtém resultados do local geo SPHome índice de pesquisa está localizado em.
...
<p>Figura 1: Pesquisando em um locatário com três locais geo</p></th>
<th align="left"><p>Cliente de pesquisa chama o ponto de extremidade do REST de pesquisa com a propriedade de consulta EnableMultiGeoSearch = true.</p>
<p>A consulta é enviada para todos os locais de geo no inquilino.</p>
<p>Resultados de pesquisa de cada local geo são mesclados e classificados.</p>
<p>Cliente obtém os resultados de pesquisa unificada.</p></th>
</tr>
</thead>
<tbody>
</tbody>
...
### Obter um centro de pesquisa para mostrar os resultados de todos os locais de geo
...
3.  Selecione a vertical para configurar, clique em **configurações do** ícone de engrenagem no canto superior direito e, em seguida, clique em **Editar página**.

> ![](media/configure-search-for-multi-geo_image2.png)
>
> A página de resultados de pesquisa é aberta no modo de edição.

1.  Na Web Part de resultados da pesquisa, mova o ponteiro para o superior, no canto direito da Web Part, clique na seta e, em seguida, clique em **Editar Web Part** no menu. ![](media/configure-search-for-multi-geo_image3.png)

> O painel de ferramentas da Web Part de resultados de pesquisa será aberto sob a faixa de opções na parte superior direita da página.
...
**Tipo_de_cliente** - esta é uma string. Insira um nome exclusivo do cliente para cada aplicativo de pesquisa. Se esse parâmetro não for incluído, a consulta é **não** leque exibindo a outros locais geo.
...
**MultiGeoSearchStatus** – esta é uma propriedade que a API de pesquisa do SharePoint retorna em resposta a uma solicitação. O valor da propriedade será uma sequência de caracteres e fornece as seguintes informações sobre a API de pesquisa do SharePoint retorna os resultados:
...
<td align="left">A consulta não foi uma consulta Multi-Geo e não foi leque exibindo para os outros locais geo.</td>
</tr>
</tbody>
</table>
