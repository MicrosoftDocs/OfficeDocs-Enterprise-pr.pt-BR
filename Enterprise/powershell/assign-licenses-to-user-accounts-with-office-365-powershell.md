...
Como você pode ver, temos um usuário não licenciado: Brenda Fernandes. Como podemos atribuir uma licença do Office 365 à Brenda?
...
Segundo, para atribuirmos uma nova licença à Brenda, precisamos saber o nome do nosso plano de licenciamento, isto é, precisamos conhecer a **AccountSkuId**. Neste caso, isso é fácil: temos somente um plano de licenciamento (`litwareinc:ENTERPRISEPACK`). No entanto, uma organização pode ter vários planos de licenciamento. Nesse cenário, **Get-MsolAccountSku** retornaria duas **AccountSkuIds** diferentes, e seria necessário escolher o plano de licenciamento apropriado ao atribuir as licenças. Por enquanto, continuaremos com o caso mais simples e consideraremos que temos apenas um plano de licenciamento.
  
Então, *como* atribuímos uma nova licença à Brenda Fernandes? Assim:
  
```
Set-MsolUserLicense -UserPrincipalName "BelindaN@litwareinc.com" -AddLicenses "litwareinc:ENTERPRISEPACK"
```

Você terá que fazer o seguinte: chamar o cmdlet **Set-MsolUserLicense**, especificando o parâmetro _UserPrincipalName_ para o usuário e o plano de licenciamento apropriado.
...
Em outras palavras, Set-MsolUserLicense não falhará simplesmente. Em vez disso, ele atribuirá todas as licenças possíveis. Só então falhará.
...
Veja outro exemplo. Talvez você queira licenciar todos os usuários no Brasil que atualmente não têm uma licença do Office 365. Nesse caso:
...
> [!NOTE]
> Quanto tempo demora para executar um comando que, digamos, emite licenças a todos os usuários não licenciados? É difícil dizer. Depende de várias fatores, desde a quantidade de usuários até a velocidade da sua conexão de rede. Se tiver algumas centenas de usuários para licenciar, será razoavelmente rápido (isto é, não deve levar mais do que um minuto ou dois). Se tiver 10 mil usuários, obviamente demorará mais. Mas não chega nem perto do tempo que demoraria para atribuir licenças a 10 mil usuários usando o Centro de administração do Office 365. 
  
Já que falamos nisso, tome cuidado com o seguinte ao atribuir licenças: se um usuário não tiver um valor configurado para a propriedade **UsageLocation**, não será possível atribuir uma licença do Office 365 a ele. Em vez disso, você receberá uma mensagem de erro similar a esta:
